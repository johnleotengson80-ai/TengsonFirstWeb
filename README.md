...
Vercel functions are stateless and have ephemeral filesystems. Product image URLs are returned as same-origin `/uploads/<filename>` paths, which work for local development and existing files on a persistent filesystem. On Vercel, product image uploads return a clear `503` unless external object storage is integrated; this avoids saving a database filename that later points to a missing `/tmp` file. `ALLOW_EPHEMERAL_UPLOADS=true` is available only as an explicit, non-durable override for testing. Durable production uploads require an object-storage adapter in `System(back-end)/routes/products.py`, and existing images must be migrated there. The current SSE implementation is process-local and is not durable or shared across serverless instances; dashboards retain their existing polling refresh and SSE should be treated as best-effort notifications.
