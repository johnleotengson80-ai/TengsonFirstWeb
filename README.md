...
The database must be external because a local SQLite file is not durable on Vercel. Create the managed database and schema before first use (the local entrypoint still runs `db.create_all()` for development). In Vercel, deploy from the project root and keep the default build settings so `vercel.json` controls the Python function and static assets.
